...
def get_pool_genesis_txn_path(pool_name,IP):
    path_temp = path_home().joinpath("pool")
    path = path_temp.joinpath("{}.txn".format(pool_name))
    save_pool_genesis_txn_file(path,IP)
    return path


def pool_genesis_txn_data(IP):
    pool_ip = environ.get("TEST_POOL_IP", IP)
...
def save_pool_genesis_txn_file(path,IP):
    data = pool_genesis_txn_data(IP)
