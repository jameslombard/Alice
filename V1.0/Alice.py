...
async def run():
    ...
    while True:
        ...
        # Send secure message:
            ...
            if IDname == '':
                IDname = await messenger(clientname)
            else:
                await messenger(clientname,IDname)

        # Create ID: 
        elif Sov=='3':

            IDname = await ID() # This step creates/loads and stores a new 

        # Create/Open wallet:
        elif Sov=='4':

            IDname = await create_wallet()

        # Create DID and Verkey for identity owner:
        elif Sov=='5':

            IDname = await did_and_verkey()            
